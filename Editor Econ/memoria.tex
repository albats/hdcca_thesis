...
The simulations yield four central findings (supplementary evidence is reported in 
Appendix 1).
...
the theoretical expectation under clean separated-spike designs. In the one-spike experiment, 
raw roots are severely inflated: for example, at \(\rho=0.6909\), \(\rho^2=0.4774\) while 
the mean raw root is \(0.6509\). In contrast, the BG-corrected estimate is \(0.4754\), 
almost unbiased. However, near or below the threshold, valid correction shares are low 
and corrected values can still be upward biased. Thus, correction is reliable 
only after separation.

Note that the robustness exercise in Table~\ref{tab:mc-exp6-summary} and 
...
% Experiment 3 - detection and bg correction results
...
between the low-rank and diffuse designs]{The figure reporst the mean leading root before 
...
% Experiment 6
\mcfigure{mc_exp6_bg_root_correction.pdf}[Experiment 6: BG-corrected leading root]{The figue reports 
...
The figure reports the observed mean angle between the obsserved and BG-predicted angles. In both cases
...
\mctable{imagenes/mc_exp6_bg_correction_summary.tex}[Experiment: 6 BG-corrected leading root summary.]{The
table reports the mean BG-corrected leading root for each design.}{tab:mc-exp6-bg-correction-summary}
\FloatBarrier
