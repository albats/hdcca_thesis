...
Thus, the statistical problem relies on the number of assets held and on 
...
patterns of co-movement, the extent to which asset returns 
...
then it is still be exposed to a common source of risk. 
...
understood as the different sets of stocks, cyclical and defensive.
...
can HDCCA theory describe the spectral and geometric structure of 
cross-sector return dependence in real large equity panels? 
...
reported in \autoref{ch:EmpiricalApplication}. 
Both the empirical analysis and simulation results are supported by reproducible code 
which can be found at \texttt{\url{https://github.com/albats/hdcca_thesis}}.
Finally, \autoref{ch:Conclusion} concludes with a discussion of implications and final remarks.
...
obtained as $a_k = \Sigma_{XX}^{-1/2} u_k$.

Analogously, the canonical directions for the \(Y_t\)-side are obtained from the
...
\Sigma_{YY}^{-1/2}.
\end{equation}
Where the eigenvalues of \(\mathcal{D}\) are the same squared canonical
correlations \(\rho_k^2\). If \(v_k\) denotes the eigenvector associated with
\(\rho_k^2\), so that $\mathcal{D}v_k = \rho_k^2 v_k$, then the corresponding canonical direction for \(Y_t\) is
$b_k = \Sigma_{YY}^{-1/2}v_k$. Subsequently, Together with $a_k$ these represent
the \(k\)-th pair of population canonical directions.
...
and measures the strongest in-sample\footnote{In-sample refers to estimations from observations in a given window.} linear association between the two panels. 

Second, to assess whether this relation is stable, the in-sample root is compared to its out-of-sample\footnote{Out-of-sample refers to 
evaluation on observations outside the estimation window. These observations that are intentionally exclided from 
estimation and reserved for such out-of-sample evaluation are called held-out data.} 
...
The empirical application uses weekly stock returns constructed from CRSP data (details in Chapter 5). 
...
in non-durables, health, telecom, and utilities.
...
Second, the number of canonical roots above the bulk edge in each window is used
to distinguish a single dominant factor from broader multi-direction dependence.

Roots above this threshold provide evidence of cross-panel dependence relative to the specified 
high-dimensional benchmark; however, the corresponding vectors must also be stable. 
A stable canonical direction would identify a reproducible long-short combination of 
cyclical stocks and a corresponding combination of defensive stocks that investors 
could monitor, hedge, or use for sector rotation. 
To measure such directional stability, each window is split into two halves and the leading canonical 
...
The fifth diagnostic is eigengap separation $\lambda_i-\lambda_{i+1}$, evaluated via the 
criterion $\lambda_{i}-\lambda_{i+1} \ge 5n^{-1/2}$, where $\lambda_{i}$ and $\lambda_{i+1}$
are the squared sample canonical roots in the window with length \(n\).
This determines whether the root is sufficiently separated to be interpreted as 
an individual component and whether BG inversion formulas can be applied credibly.  
Finally, out-of-sample performance is evaluated: canonical weights estimated in one part of 
the data are applied to the other 
% and the held-out leading squared canonical correlation is recorded 
to show whether the apparent dependence survives.
...
The baseline specification uses rolling windows of a fixed length of 260 weeks and step size of 4 weeks, yielding
144 windows. This allows the canonical spectrum to vary over time while keeping the 
setting explicitly high-dimensional. For each window, the diagnostics are computed and outlier roots are treated as
credibly identified if sufficiently separated from the bulk and each other.
...
mean HDCCA bulk edge of 0.8521. It exceeds the bulk edge in every window, and the 
mean number of roots above the bulk is 4.17. This is supported by Figure~\ref{fig:emp-leading-root-null}
where the leading root is consistently too large to be explained
by high-dimensional noise alone, as represented by the permutation null. 
This indicates that cyclical and defensive stocks share strong common variation, so the 
two groups are not independent sources of portfolio risk. Figure~\ref{fig:emp-outlier-count} 
further shows that the evidence is not confined to a single isolated component as multiple roots lie 
above the bulk in all windows, indicating that the cross-panel dependence is multidimensional 
at the spectral level, rather than being driven only by the first canonical relation.
...
The lack of stability implies that the estimated canonical directions should not be treated 
as reliable portfolio signals. The co-movement found in the historical data may be driven 
by temporary macroeconomic conditions or noise, rather than by stable relationships 
between cyclical and defensive stocks. Given that canonical directions estimated in one 
period do not hold out of sample, their usefulness for building a reliable rotation 
strategy between the two groups is limted.
...
baseline run, there are zero credible leading windows out of 144, that is. no rolling window satisfies the 
...
Thus, evidence suggests persistent multi-dimensional dependence, 
...
dependence between cyclical and defensive stock returns which is not plausibly attributable to noise alone. 
...
Economically, this entails that the cyclical-defensive classification remains relevant as
the persistent spectral outliers show that cyclical and defensive stocks are linked by 
common market forces. For a diversified investor, this weakens the interpretation of 
defensive stocks as an independent hedge against cyclical exposure. 
Even when the portfolio is split across economically different sectors, part of the risk 
remains shared across the two panels.

At the same time, the instability of the canonical directions limits the usefulness of CCA 
as a direct portfolio-construction tool. If the leading direction were stable, it could be 
interpreted as a reproducible consistent relationship between specific combinations of 
cyclical and defensive stocks. Such weights could then be applied to monitor cross-sector 
risk. However, the empirical results do not support this 
interpretation as the leading directions change substantially 
across samples and does not generalize out of sample. Moreover, the failure to apply the 
BG correction formulas credibly reinforces the same point, the leading canonical component is 
not sufficiently well separated to be treated as a robust economic relationship.

The practical implication is therefore that, while HDCCA 
reveals that diversification between cyclical and defensive stocks may be 
less effective than the sector labels suggest, it is not useful here as a stable 
signal-extraction device for identifying a single tradable cross-sector factor. 
...
They confirm the baseline message that the spectrum consistently indicates cross-sector 
dependence, but the leading canonical portfolio is not a well-identified economic direction under reasonable 
alternative specifications.

Both with shorter 208-week windows and longer 364-week windows, the leading root remains an 
outlier in every window, but vector stability, top-5 subspace overlap, and held-out performance all remain very weak, with no credible 
leading windows. Stronger spectral separation is found in longer windows. These allow a larger part 
of the broad cross-panel dependence to survive out of sample, which is consistent with lower estimation noise and 
with more persistent low-frequency co-movement over longer horizons. Nonetheless, tehy still
fail to deliver stable leading canonical components that could be interpreted
...
leading canonical relation but also in weaker ranks. 
...
This paper studied if HDCCA provides a useful framework for describing 
cross-sector dependence in large equity panels. The analysis focused on 
the relationship between cyclical and defensive stock returns, 
combining Monte Carlo simulations with an application to CRSP data.

The Monte Carlo simulations show that in high-dimensional settings, large sample 
canonical roots may arise even when the two panels are independent. 
Moreover, under true dependence spectral separation appears before 
reliable directional recovery. This is central to the interpretation of HDCCA 
as canonical roots may indicate the presence of cross-panel dependence even 
when the associated canonical directions remain too unstable to support a 
clear economic interpretation.

This is precisely what the empirical application finds. In
financial data, HDCCA provides a warning against naive diversification claims, as the 
canonical spectrum displays persistent dependence between 
cyclical and defensive stocks. The strong spectrum suggests that cyclical and 
defensive allocations should not be treated as automatically diversified, 
especially when aggregate shocks affect both groups. However, the leading canonical 
directions are not recovered as a stable economic direction. 
Vector stability is low, out-of-sample performance weak, and the 
leading root is not sufficiently separated from nearby roots to 
recover.

In practice, HDCCA helps explain why large canonical roots in 
high-dimensional data must be interpreted cautiously. Notably for 
portfolio construction, risk management, or factor interpretation,
in-sample HDCCA outliers are not necessarily economically meaningful
if they are not also stable across samples and persist out of sample. 
Otherwise, the estimated canonical directions may reflect
...
Several limitations apply as the empirical analysis is narrow, focusing on a single panel design, 
frequency, and the HDCCA benchmark. It does not compare alternative
