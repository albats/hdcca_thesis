...
sectors to capture rising market momentum (\cite{deLongisZaninEllis2022}).
...
(\cite{StanglJacobsenVisaltanachoti2007}). This shifting balance is 
...
patterns of co-movement\footnote{Note that in this thesis, co-movement refers to linear 
dependence between return panels, measured through canonical correlations between linear 
combinations of cyclical and defensive stock returns.}, the extent to which asset returns 
...
portfolio, the cyclical and defensive stocks may move largely independently, so the 
defensive portion can help offset losses during downturns. In the other, the two groups 
may move closely together, meaning that losses in one part of the portfolio are likely to 
be accompanied by losses in the other. The difference between these portfolios is therefore 
not visible from the allocation alone; it lies in the pattern of relationships between the 
assets.

The economic forces that may drive this co-movement can arise due to changes in interest 
rates, inflation, investor confidence, or market-wide uncertainty, which affect many 
sectors at once. Therefore, the relationship between cyclical and defensive stocks is not 
...
maximize their cross-correlation. In this paper, the two groups may be understood as 
different sets of stocks, such as cyclical and defensive stocks.
...
high-dimensional CCA, where the number of variables in the two datasets grows together 
...
This distinction is directly related to diversification. If the relationship between cyclical 
and defensive stocks is largely explained by a small number of common patterns, then the 
portfolio may be more exposed to shared economic forces than its sector labels suggest. If 
...
meaningfully describe the structure of \textit{real} equity markets. This paper seeks to 
address this gap in the literature by answering the following question: to what extent 
...
of high-dimensional CCA theory in real financial markets. Second, it clarifies whether 
...
financial dimensions.  These simulations act as a stepping stone to \autoref{ch:Methodology}, 
as they provide the intuition behind the benchmarks and the diagnostic framework in the 
...
In the context of this paper, these vectors represent panels of cyclical and defensive 
...
For the optimization problem defined in Equation [2.1], as demonstrated by \cite[pp.~539--574]{johnson2007applied} 
in Chapter 10, solving this constrained maximization through the method of Lagrange multipliers 
is equivalent to solving a generalized eigenvalue problem. Specifically, the weights are 
...
obtained as $a_k = \Sigma_{XX}^{-1/2} u_k$. An analogous representation holds for $Y_t$. 

The same result can also be expressed through the singular value decomposition of the 
...
The squared sample canonical correlations are therefore the eigenvalues of 
$\widehat{\mathcal{C}} 
= S_{XX}^{-1/2} S_{XY} 
S_{YY}^{-1} 
S_{YX} S_{XX}^{-1/2}$.
...
distributions for canonical correlations can be derived. In this regime, CCA provides a 
...
Addtionally, when a finite number of population canonical correlations are present, 
...
These phenomena are formalized in \emph{High-Dimensional Canonical Correlation Analysis} 
by \citet{BykhovskayaGorin2023}. They consider a regime in which
\[
p, q, n \to \infty
\quad \text{with} \quad
...
The existence of this critical threshold separating two regimes (the bulk, where weak 
...
Given observed empirical canonical correlations and known dimensional ratios $(p/n, q/n)$, 
...
This paper evaluates whether the structural patterns they show for high-dimensional 
...
% Throughout these sections, a canonical root denotes a squared canonical correlation, \(\lambda_k=\rho_k^2\). 
...
The simulations yield three central findings (supplementary evidence is reported in 
Appendix 1).
...
Table~\ref{tab:mc-exp1-summary} and Figures~\ref{fig:mc-exp1-scree}--\ref{fig:mc-exp1-hist} 
show that the leading root is centered around 0.50, whereas the out-of-sample root 
and split-sample overlaps are essentially zero. This illustrates that large in-sample roots 
...
The one-spike sweep in Table~\ref{tab:mc-exp2-summary} and 
Figures~\ref{fig:mc-exp2-spectral}--\ref{fig:mc-exp2-angles} show that weak population spikes 
...
Table~\ref{tab:mc-exp3-summary} and Figures~\ref{fig:mc-exp3-detection}--\ref{fig:mc-exp3-angles} 
shows that crossing \(\rho_c \approx 0.426\) (the critical threshold) does not produce a sharp 
...
Third, the multi-spike and diffuse designs show that detecting the signals also depends on how 
they are distributed. In the multiple-spike design, only the strongest components separate most 
clearly, while weaker components are harder to distinguish from the bulk; see 
Tables~\ref{tab:mc-exp4-summary}--\ref{tab:mc-exp4-components} and Figure~\ref{fig:mc-exp4-scree}. 
The low-rank versus diffuse comparison in Figures~\ref{fig:mc-exp5-lowrank}--\ref{fig:mc-exp5-diffuse}
portrays this point as concentrated dependence produces clearer leading roots and greater directional
stability (see Table~\ref{tab:mc-exp5-summary}), whereas diffuse dependence 
produces a smoother spectrum that remains closer to the high-dimensional bulk.

Finally, the robustness exercise in Table~\ref{tab:mc-exp6-summary} and 
Figure~\ref{fig:mc-exp6-robustness} shows that the main conclusions survive moderate 
covariance misspecification. Detection remains strong when the clean 
Gaussian benchmark is relaxed and introducing within-panel covariance has limited effects in this 
calibration, whereas heavy-tailed observations do increase the in-sample root and weaken 
directional recovery and stability.
...
canonical directions and held-out performance. They also show that the presence of several 
...
separation, and instability of the estimated canonical directions. As shown in Appendix 1, 
...
in-sample fit alone and instead evaluates spectral strength, stability, and out-of-sample 
performance jointly.
...
an individual component. This step is essential as, when eigengaps are too small, the Bykhovskaya-Gorin 
...
The final sample contains 832 weekly observations running from 8 January 2010 to 12 December 2025, 
together with two balanced cross-sections of 80 cyclical and 80 defensive stocks. 
In the final panel, the cyclical group is composed mainly of retail/services, manufacturing, 
energy, and chemicals, while the defensive group is concentrated in health, utilities, non-durables, and telecom. 
This composition is intended to ensure that the two panels represent distinct sectors of the economy and differ in their 
...
0.0329 on the defensive side, as reported in Table~\ref{tab:emp-main-summary} and visualized in 
...
This is exactly the gap highlighted by the Monte Carlo analysis. A large in-sample canonical root is not enough
to be economically meaningful; the corresponding dependence must also survive out of sample.
...
Accordingly, a phase-transition recovery exercise cannot be performed as the leading roots are spectrally 
strong but geometrically fragile. This finding is itself consistent with the HDCCA perspective. 
...
dependence between cyclical and defensive stock returns. It is not plausibly attributable to noise alone. 
But at the same time, the leading empirical canonical portfolio is not a stable object as the estimated 
...
by the specific pre-/post-2020 sample split as seen in Table~\ref{tab:emp-robustness-summary} and Figure~\ref{fig:emp-robustness}.

With shorter 208-week windows, the leading root remains an outlier in every window, but vector stability, 
top-5 subspace overlap, and held-out performance all remain very weak, and there are again no credible 
leading windows. With longer 364-week windows, spectral separation becomes stronger and the mean held-out 
...
and the number of credible leading windows is still zero, failing to deliver a stable leading canonical portfolio 
that could be interpreted as a clean, well-identified factor. 

The subsample analysis leads to the same conclusion. In both the pre-2020 and post-2020 samples, the leading 
root remains an outlier throughout, but overlap measures remain low, held-out roots remain small, 
and no credible leading windows emerge.
...
\subsection*{Monte Carlo Simulation - Experiment 1: Pure-Noise Benchmark}
\mctable[0.45\textwidth]{imagenes/mc_exp1_summary.tex}[Experiment 1 summary statistics under the pure-noise benchmark.]{ }{tab:mc-exp1-summary}
\mcfigure{mc_exp1_scree.pdf}[Scree plot for the mean canonical roots for the pure-noise scenario (Experiment 1).]{ 
The scree plot figure displays the ordered canonical roots, \(\widehat{\lambda}_1 \geq \widehat{\lambda}_2 \geq \cdots\), where 
\(\widehat{\lambda}_k=\widehat{\rho}_k^2\) (the \emph{rank} in the x-axis refers to the position of 
each canonical root after sorting them from largest to smallest) and it can be used to assess whether the leading 
canonical roots separate from the rest of the spectrum. In the pure-noise scenario it displays a smooth, bulk-like structure.}{fig:mc-exp1-scree}
\mcfigure{mc_exp1_histogram.pdf}[Experiment 1 pure-noise distribution of the leading sample canonical correlation.]{ 
The vertical lines report the 95th and 99th percentiles of the leading-root distribution generated. }{fig:mc-exp1-hist}
\FloatBarrier

\subsection*{Monte Carlo Simulation - Experiment 2: One-Spike Signal-Strength Sweep}
\mctable{imagenes/mc_exp2_summary.tex}[Experiment 2 summary table for the one-spike signal-strength sweep.]{ }{tab:mc-exp2-summary}
...
\mcfigure{mc_exp2_stability.pdf}[Experiment 2 out-of-sample strength and split-sample stability.]{
The figure reports the held-out leading canonical root and the split-sample directional overlaps. 
Both diagnostics remain close to zero for weak signals and become substantial only for sufficiently strong population 
canonical correlations, showing that in-sample separation alone does not guarantee stable or generalizable canonical structure.}{fig:mc-exp2-stability}
...
\subsection*{Monte Carlo Simulation - Experiment 3: Threshold Zoom}
...
\mcfigure{mc_exp3_angles.pdf}[Experiment 3 vector recovery around the threshold.]{
The figure reports the angle between the estimated and true leading canonical directions for the \(X\)- and \(Y\)-side panels. 
Angles decline as the population canonical correlation \(\rho\) increases, but remain large near the critical 
value \(\rho_c\), indicating weak directional recovery in the near-threshold region.}{fig:mc-exp3-angles}
\FloatBarrier

\subsection*{Monte Carlo Simulation - Experiment 4: Multiple-Spike Low-Rank Design}
\mctable[0.45\textwidth]{imagenes/mc_exp4_summary.tex}[Experiment 4 summary statistics for the three-spike low-rank design.]{ }{tab:mc-exp4-summary}
\mctable[0.65\textwidth]{imagenes/mc_exp4_components.tex}[Experiment 4 component-by-component recovery measures.]{ }{tab:mc-exp4-components}
...
\subsection*{Monte Carlo Simulation - Experiment 5: Diffuse Versus Low-Rank Dependence}
\mctable{imagenes/mc_exp5_summary.tex}[Experiment 5 comparison of the low-rank and diffuse designs.]{ }{tab:mc-exp5-summary}
...
\subsection*{Monte Carlo Simulation - Experiment 6: Finance-Style Robustness Checks}
...
\mcfigure{mc_exp6_robustness.pdf}[Experiment 6 robustness comparison across finance-style data-generating processes.]{
The Gaussian spiked-covariance case changes the within-panel covariance structure, 
while the Student-\(t\) case changes the tail behavior of the observations. 
Detection remains strong across the three designs, as reflected in large leading in-sample roots. 
However, the Student-\(t\) design produces weaker directional recovery, shown by a larger 
average angle for the \(X\)-side canonical vector, and lower stability relative to the Gaussian cases.}{fig:mc-exp6-robustness}
\FloatBarrier
...
\mctable[0.62\textwidth]{imagenes/emp_main260_summary.tex}[Baseline 260-week rolling summary for the CRSP empirical application.]{ }{tab:emp-main-summary}
...
isolated windows.}{fig:emp-leading-root-null}
...
part of the sample. This may suggest that the empirical dependence is not limited to the first canonical root.}{fig:emp-outlier-count}
...
weight vectors are not consistently recovered across subsamples, despite the large in-sample canonical root.}{fig:emp-split-stability}
...
out-of-sample dependence.}{fig:emp-insample-heldout}
...
of the leading canonical subspaces.}{fig:emp-topk}
...
interpret as individually well-identified components.}{fig:emp-eigengap}
\emptable{imagenes/emp_robustness_summary.tex}[Robustness summary across alternative rolling-window lengths and subsamples.]{
...
the stablished credibility criterion under these robustness checks.}{tab:emp-robustness-summary}
\empfigure{../CRSP_Analysis/empirical_outputs_crsp/figures/figure8_robustness_summary_across_specs.png}[Cross-specification robustness comparison for spectral strength, stability, and held-out dependence.]{ }{fig:emp-robustness}
\setcounter{savedempfigure}{\value{figure}}
\setcounter{savedemptable}{\value{table}}
...
\chapter*{Appendix}
\section*{Appendix 1: Monte Carlo Full Results}
\label{app:mc-results}
...
repository at \url{https://github.com/albats/hdcca_thesis}, with the Monte Carlo notebook located in 
...
\clearpage
\section*{Appendix 2: Empirical Application Supplementary Results}
\label{app:emp-results}
\setresultnumbering{5}
\setcounter{figure}{\value{savedempfigure}}
\setcounter{table}{\value{savedemptable}}

This appendix collects the supplementary tables and figures for the CRSP empirical application in 
Chapter~\ref{ch:EmpiricalApplication}. Table~\ref{tab:emp-baseline-sample} reports the baseline 
sample design used in the rolling analysis. Table~\ref{tab:emp-full-spectrum} and 
Figure~\ref{fig:emp-full-spectrum} summarize the full-sample static spectrum, which confirms that 
the presence of multiple outliers is not solely a rolling-window artifact. 

\emptable[0.48\textwidth]{imagenes/emp_baseline_sample.tex}[Baseline CRSP sample design used in the empirical rolling-window application.]{ }{tab:emp-baseline-sample}
\emptable[0.42\textwidth]{imagenes/emp_full_spectrum_top10.tex}[Top ten squared canonical roots from the full-sample static spectrum, with the HDCCA bulk-edge classification.]{ }{tab:emp-full-spectrum}
\empfigure{../CRSP_Analysis/empirical_outputs_crsp/figures/figure_full_sample_static_spectrum.png}[Full-sample static canonical spectrum for the CRSP cyclical and defensive panels.]{ }{fig:emp-full-spectrum}
